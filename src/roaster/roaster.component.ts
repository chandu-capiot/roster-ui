...
export class RoasterComponent implements OnInit, AfterContentInit {
  ...
  ngOnInit() {
    ...
    setTimeout(function () {
      ...
      $(document).mouseup(function (e) {
        const container = $('.event-info-modal');
  ...
  timelineWeekHeader(startDate) {
    ...
    for (let i = 1, colspan = 7; i <= colspanCount / 7; i++) {
      ...
      row.append('<th colspan="' + colspan + '" class="week-str">' + currentWeek
        + this.getWeekStr(weekStartDate, weekEndDate, 'week') + ' </th>');
    }
  ...
  getResourceProjectChildrenDOM(resourceObj) {
    const resourceHTML = '<div class="container resource-level-3">' +
      ...
      '<div class="fc-cell-text col-md-9 text-right resource-name">' + resourceObj.name
      + '<img src="assets/images/avatar.png" class="profile-photo"></div>' +
      '</div>' +
      '</div>';
    return resourceHTML;
  }
